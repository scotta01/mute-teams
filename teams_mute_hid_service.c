#include "teams_mute_hid_service.h"

#include <furi.h>
#include <stdint.h>
#include <string.h> // memcpy

// Generic, FAP-exposed GATT API + the BLE constants the service needs.
#include <ble/core/ble_defs.h> // UUID_TYPE_16, PRIMARY_SERVICE, CHAR_PROP_*, ATTR_*, GATT_*, CHAR_VALUE_LEN_*
...
static BleEventAckStatus ble_svc_hid_event_handler(void* event, void* context) {
    ...
    /*
     * The stock service inspects raw ST HCI events (hci_uart_pckt /
     * evt_blecore_aci) here to ack attribute-modified and server-confirmation
     * events. Those types are intentionally not exposed by the public SDK, and
     * this remote only SENDS input-report notifications, so we defer every
     * event to the firmware's default handler by returning BleEventNotAck.
     */
